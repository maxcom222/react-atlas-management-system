...
import { deleteCPrevServ, setCPrevServ } from "../store";
...
const ToolbarContent = (props) => {
    ...
    const { cprev_services } = useSelector(state => state.crewApp.crew_details);
     ...
     const handleDelete = (event) => {
         if(cprev_services.recent) {
             if(window.confirm("Are you sure to delete this item?")){
                 dispatch(deleteCPrevServ(cprev_services.recent));
                 dispatch(setCPrevServ(null));
             }   
         }    
     }
