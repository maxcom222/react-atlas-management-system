...
import { deletePrevServ, setPrevServ } from "../store";
...
const ToolbarContent = (props) => {
    ...
    const {prev_services} = useSelector(state => state.crewApp.crew_details);
    ...
    const handleDelete = (event) => {
        if(prev_services.recent) {
            if(window.confirm("Are you sure to delete this item?")){
                dispatch(deletePrevServ(prev_services.recent));
                dispatch(setPrevServ(null));
            }   
        }    
    }
