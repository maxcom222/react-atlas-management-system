...
import { deleteAppraisals, setAppraisals } from "../store";
...
const ToolbarContent = (props) => {
    ...
    const { appraisals } = useSelector(state => state.crewApp.crew_details);
     ...
     const handleDelete = (event) => {
         if(appraisals.recent) {
             if(window.confirm("Are you sure to delete this item?")){
                 dispatch(deleteAppraisals(appraisals.recent));
                 dispatch(setAppraisals(null));
             }   
         }    
     }
